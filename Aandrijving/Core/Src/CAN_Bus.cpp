...
void CANBus::transmit(CAN_HandleTypeDef *hcan, uint32_t can_id, uint8_t *TxData, uint8_t length) {
    CAN_TxHeaderTypeDef TxHeader;
    uint32_t TxMailbox;

    // Prepare the CAN frame header
    TxHeader.StdId = can_id;         // Set CAN ID
    TxHeader.ExtId = 0;              // Not using extended ID
    TxHeader.RTR = CAN_RTR_DATA;     // Data frame
    TxHeader.IDE = CAN_ID_STD;       // Standard frame ID
    TxHeader.DLC = length;           // Length of the data
    TxHeader.TransmitGlobalTime = DISABLE;  // Disable timestamping

    // Send the message using HAL CAN function
    if (HAL_CAN_AddTxMessage(hcan, &TxHeader, TxData, &TxMailbox) != HAL_OK) {
        // Handle error if the message cannot be sent
        // You can add some error handling code here if required
    }
}
