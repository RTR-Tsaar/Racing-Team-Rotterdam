...
Encoder::Encoder(MotorControl* motorControl, CurrentSensor* currentSensor, TIM_HandleTypeDef* enc1, TIM_HandleTypeDef* enc2, uint16_t maxCount)
    : motorControl(motorControl), currentSensor(currentSensor), encoder1(enc1), encoder2(enc2), maxEncoderCount(maxCount) {
    // Constructor
};
